...
- **Lazy SMP**: at the start of each search, up to `n_helpers` background threads are spawned (physical CPUs − 1 by default; override with `N_HELPERS=n`). If `n_helpers > 0`, the first slot is taken by a **PV-leaf helper**: after each completed depth it receives the main thread's best PV, validates each move against the legal move set (to guard against stale or race-corrupted PVs), plays to the leaf, then runs iterative deepening from there — pre-populating TT entries for lines the main thread is about to search. The remaining `n_helpers − 1` slots run standard Lazy SMP helpers that each independently run the full iterative-deepening loop from the root, sharing only the transposition table; TT races cause natural divergence in move ordering. When the main thread finishes or is cancelled, it sets `is_cancel_pending` and joins all helpers. Compile with `SERIAL=1` to disable all parallel search.
