...
void RunPositionTests(int depth)
{
    ...
    int prevClockType = globals->timeControl.clockType;
    int prevDepth     = globals->timeControl.fixedDepth;
    globals->timeControl.clockType  = FIXED_DEPTH;
    globals->timeControl.fixedDepth = depth;
    ...
    printf("total elapsed milliseconds                        %10ld\n", GetMilliseconds() - firstStart);
    globals->timeControl.clockType  = prevClockType;
    globals->timeControl.fixedDepth = prevDepth;
}
