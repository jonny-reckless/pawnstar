...
static void DisplayBitboard(bitboard whitePawns, bitboard blackPawns, bitboard white, bitboard black, const char* name)
{
    int x, y;
    printf("\n                       %s\n", name);
    for (y = 7; y != -1; --y)
    {
        ...
        for (x = 0; x != 8; ++x)
        {
            const bitboard square = BITBOARD_XY(x, y);
            printf(whitePawns & square ? "W " : blackPawns & square ? "B " : "+ ");
        }
        printf(" %d  ", y + 1);
        for (x = 0; x != 8; ++x)
        {
...
void DisplayPawnStructure(const Position* position)
{
    ...
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].pawnAttacks,      ps[BLACK].pawnAttacks,      "pawn attacks");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].defendedPawns,    ps[BLACK].defendedPawns,    "defended pawns");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].pawnHoles,        ps[BLACK].pawnHoles,        "pawn holes");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].outposts,         ps[BLACK].outposts,         "outposts");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].isolatedPawns,    ps[BLACK].isolatedPawns,    "isolated pawns");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].doubledPawns,     ps[BLACK].doubledPawns,     "doubled pawns");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].blockedPawns,     ps[BLACK].blockedPawns,     "blocked pawns");
    DisplayBitboard(whitePawns, blackPawns, ps[WHITE].passedPawns,      ps[BLACK].passedPawns,      "passed pawns");
}
