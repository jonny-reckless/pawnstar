...
struct RawWeights
{
    std::vector<int16_t> feature_weights; // kFeatureRows * kHiddenSize
    std::vector<int16_t> feature_bias;    // kHiddenSize
    std::vector<int16_t> output_weights;  // 2 * kHiddenSize
...
bool ParseNet(const std::string &path, RawWeights &w)
{
    ...
    w.feature_weights.resize(nfw);
    w.feature_bias.resize(kHiddenSize);
    w.output_weights.resize(2 * kHiddenSize);
    auto take = [&](void *dst, std::size_t count) {
        std::memcpy(dst, buf.data() + off, count * sizeof(int16_t));
        off += count * sizeof(int16_t);
    };
    take(w.feature_weights.data(), nfw);
    take(w.feature_bias.data(), kHiddenSize);
    take(w.output_weights.data(), 2 * kHiddenSize);
    std::memcpy(&w.output_bias, buf.data() + off, sizeof(int16_t));
    return true;
}
...
struct Dist
{
    ...
    void                report(const char *name)
    {
        ...
        auto         pct  = [&](double p) { return xs[std::min<std::size_t>(xs.size() - 1, (std::size_t)(p * n))]; };
        std::printf("  %-22s n=%-10zu min=%.0f  p50=%.0f  p95=%.0f  p99=%.0f  p99.9=%.0f  max=%.0f  mean=%.1f sd=%.1f\n",
                    name, xs.size(), xs.front(), pct(0.50), pct(0.95), pct(0.99), pct(0.999), xs.back(), mean, sd);
    }
};
...
int main(int argc, char **argv)
{
    ...
    int max_ow = 0;
    for (int16_t v : w.output_weights)
        max_ow = std::max(max_ow, std::abs((int)v));
    int max_fw = 0;
    for (int16_t v : w.feature_weights)
    ...
    for (const Position &pos : positions)
    {
        ...
        net.Refresh(acc, pos);
        const bool wtm = pos.color_to_move == kWhite;
        const std::array<int16_t, kHiddenSize> &stm = wtm ? acc.white : acc.black;
        const std::array<int16_t, kHiddenSize> &ntm = wtm ? acc.black : acc.white;
        ...
        for (int i = 0; i < kHiddenSize; ++i)
        {
            ...
            raw += (std::int64_t)cs * cs * w.output_weights[i];
            raw += (std::int64_t)cn * cn * w.output_weights[kHiddenSize + i];
        }
        ...
        for (int S = kSmin; S <= kSmax; ++S)
        {
            ...
            for (int i = 0; i < kHiddenSize; ++i)
            {
                const int ws_i  = Clamp((int)std::lround((double)w.output_weights[i] / WS), -127, 127);
                const int wn_i  = Clamp((int)std::lround((double)w.output_weights[kHiddenSize + i] / WS), -127, 127);
            ...
            double       out        = approx_raw / kQA + w.output_bias;
        ...
        for (int Sp = kSmin; Sp <= kSmax; ++Sp)
        {
            ...
            auto probe = [&](const std::array<int16_t, kHiddenSize> &a, int woff) {
                ...
                {
                    const int w0 = Clamp((int)std::lround((double)w.output_weights[woff + i] / WS), -127, 127);
                    const int w1 = Clamp((int)std::lround((double)w.output_weights[woff + i + 1] / WS), -127, 127);
    ...
    const double max_dot = dot_dist.xs.empty() ? 0 : dot_dist.xs.back();
    ...
    if (moves.size() == 0)
        base = Position::FromString("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"), moves = base.GenerateLegalMoves();
    ...
    for (long i = 0; i < kEvalIters; ++i)
        sink += net.Evaluate(acc, base.color_to_move);
    auto t3 = now();
