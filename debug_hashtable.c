...
static DebugEntry debugDict[DEBUG_DICT_SIZE];
/******************************************************************************
Initialize or reset the dictionary
*******************************************************************************/
void DebugXClear()
{
    memset(debugDict, 0, sizeof(debugDict));
}
/******************************************************************************
...
void DebugXIncrement(const char key[])
{
    ...
    */
    DebugEntry* const entry = &debugDict[((uint64)key >> 2) % DEBUG_DICT_SIZE];
...
void DebugXWrite(FILE* file)
{
    ...
    for (entry = debugDict; entry != debugDict + DEBUG_DICT_SIZE; ++entry)
    {
        if (entry->key)
        {
