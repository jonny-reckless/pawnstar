...
static int pieceSquareValues[2][8][64];
static int kingEndgameValues[2][64];
...
void InitializePieceSquareTable()
{
    ...
    for (location = A1; location <= H8; ++location)
    {
        ...
        {
            pieceSquareValues[WHITE][piece][location] = MATERIAL_VALUES[piece] + PIECE_SQUARES[piece][location ^ RANK_FLIP];
            pieceSquareValues[BLACK][piece][location] = MATERIAL_VALUES[piece] + PIECE_SQUARES[piece][location];
        }
        kingEndgameValues[WHITE][location] = KING_SQUARE_ENDGAME[location ^ RANK_FLIP];
        kingEndgameValues[BLACK][location] = KING_SQUARE_ENDGAME[location];
    }
}
/******************************************************************************
...
{    
    ...
    {
        bitboard b = position->pieces[piece] & position->whitePieces;
        while (b)
        {
            score += pieceSquareValues[WHITE][piece][FindAndClearLsb(&b)];
        }
        b = position->pieces[piece] & position->blackPieces;
        while (b)
        {
            score -= pieceSquareValues[BLACK][piece][FindAndClearLsb(&b)];
        }
    }    
    /* Endgame? */
    if (!position->queens || 
        PopCount(position->occupiedSquares ^ position->pawns) < 8)
    {
        score += kingEndgameValues[WHITE][position->kingLocation[WHITE]];
        score -= kingEndgameValues[BLACK][position->kingLocation[BLACK]];
    }
    else
    {
        score += pieceSquareValues[WHITE][KING][position->kingLocation[WHITE]];
        score -= pieceSquareValues[BLACK][KING][position->kingLocation[BLACK]];
    }
