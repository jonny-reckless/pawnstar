...
static inline __attribute__((always_inline)) square_t square_from_coords(int file, int rank)
{
    return (square_t)(file + 8 * rank);
}

/// @brief Construct from name string e.g. "e4".
static inline __attribute__((always_inline)) square_t square_from_string(const char *str)
{
    return (square_t)(((str[0] | 0x20) - 'a') + 8 * (str[1] - '1'));
}

/// @brief File (0..7 → a..h).
static inline __attribute__((always_inline)) int square_file(square_t s)
{
    return s & 7;
}

/// @brief Rank (0..7).
static inline __attribute__((always_inline)) int square_rank(square_t s)
{
    return s >> 3;
}
