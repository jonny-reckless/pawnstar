...
#define COLOR_TO_MOVE(position)         (((position)->stateFlags & IS_BLACK_TO_MOVE) ? BLACK : WHITE)
#define COLOR_NOT_TO_MOVE(position)     (((position)->stateFlags & IS_BLACK_TO_MOVE) ? WHITE : BLACK)
#define HAS_SINGLE_BIT_SET(x)           ((x) && !((x) & ((x) - 1)))
#define COLOR_AT(position, location)    ((BITBOARD(location) & (position)->whitePieces) ? WHITE : \
                                        ( BITBOARD(location) & (position)->blackPieces) ? BLACK : NEITHER_COLOR)
