...
- `test_nnue` (`test/nnue_test.cpp`) takes `<net.bin> <reference.txt>` and checks the engine's NNUE eval against the trainer's reference evals (within ±2 cp; the checked-in `test/nnue_reference.txt` is 250 evals for `pawnstar-v8.bin`, generated by the bullet `pawnstar_eval` example — regenerate it if you ship a new net). With no arguments it is a no-op.
...
- **Weights** are bullet's native quantised payload (tightly packed little-endian `int16`: `feature_weights[768*1*1024]` bucket-major, `feature_bias[1024]`, `output_weights[2048]`, scalar `output_bias`; bullet pads the trailing tensor to 64 bytes). The forward pass and dequantisation mirror bullet's example exactly; correctness is verified by `test_nnue`.
