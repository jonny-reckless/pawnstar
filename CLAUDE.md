...
**Lazy SMP** — At the start of each `search_root_node` call, up to `n_helpers` threads are spawned via `thrd_create`. If `n_helpers > 0`, the first slot is a **PV-leaf helper** (`lazy_pv_helper_fn`): the main thread publishes its best PV into `pv_helper_arg_t::pv` and increments `pv_helper_arg_t::generation` with `memory_order_release` after each completed depth; the helper reads `generation` with `memory_order_acquire`, snapshots the PV, then validates each move against `position_generate_legal_moves` before playing it (guarding against stale or race-corrupted PVs that could otherwise leave the position illegal). After advancing to the leaf it runs iterative deepening from there, populating TT entries for lines the main thread is about to visit. The remaining `n_helpers − 1` slots are standard Lazy SMP helpers (`lazy_helper_fn`) that each independently run the full iterative-deepening loop from START_DEPTH. All threads share only the transposition table; TT races cause natural divergence in move ordering. When the main thread finishes or times out, it stores `is_cancel_pending = true` and `thrd_join`s all helpers. Parallel search is gated by `#if DO_PARALLEL_SEARCH` (default 1; disable with `SERIAL=1` make flag).
