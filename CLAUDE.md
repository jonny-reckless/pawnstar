...
./build/test_bratko_kopec_nnue nnue/pawnstar-v2.bin   # same 24 positions searched with NNUE; reports moves solved
./build/test_nnue nnue/pawnstar-v2.bin test/nnue_reference.txt   # NNUE inference cross-check vs trainer
./build/test_nnue_incremental nnue/pawnstar-v2.bin              # incremental accumulator == full refresh
```

`make check` runs both NNUE tests against the shipped `nnue/pawnstar-v2.bin` (the Makefile passes the net via `$(wildcard …)`, so the checks degrade to a green no-op only if the net/reference are absent):
- `test_nnue` (`test/nnue_test.cpp`) takes `<net.bin> <reference.txt>` and checks the engine's NNUE eval against the trainer's reference evals (within ±2 cp; the checked-in `test/nnue_reference.txt` is 250 evals for `pawnstar-v2.bin`, generated by the bullet `pawnstar_eval` example — regenerate it if you ship a new net). With no arguments it is a no-op.
