...
- **Architecture**: a simple perspective net with the standard bullet `Chess768` feature set — `768 → 512` feature transformer (shared across both perspectives), SCReLU activation, `concat[stm | ntm] = 1024 → 1` output (centipawns, side-to-move relative). Constants: `kHiddenSize=512`, `kQA=255`, `kQB=64`, `kScale=400`.
...
The repo ships one net ([nnue/README.md](nnue/README.md) §7): `nnue/pawnstar-v4.bin`, a **512-wide** net trained on public PlentyChess data that *beats* the HCE by a wide margin. Lineage: self-play labels lost to HCE (label quality was the lever); ~12× more data gave nothing (the 256 net was capacity-limited); doubling width to 512 added +55 Elo (fixed depth) / +71 (time control). Off by default. Incremental accumulator + AVX2 SIMD keep it fast enough to win on the clock, not just at equal depth. A 512 engine cannot load a 256 net (size-gated), so the old 256 v1/v2 nets were retired.
