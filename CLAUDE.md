...
./build/test_bratko_kopec_nnue nnue/pawnstar-v8.bin   # 24 Bratko-Kopec positions searched with NNUE; reports moves solved
./build/test_nnue nnue/pawnstar-v8.bin test/nnue_reference.txt   # NNUE inference cross-check vs trainer
...
`make check` runs the NNUE tests against the shipped `nnue/pawnstar-v8.bin` (the Makefile passes the net via `$(wildcard …)`, so the checks degrade to a green no-op only if the net/reference are absent):
- `test_nnue` (`test/nnue_test.cpp`) takes `<net.bin> <reference.txt>` and checks two things per position: (1) the **exact int16 reference** (`Network::EvaluateExact`) matches the trainer's reference evals within ±2 cp (the checked-in `test/nnue_reference.txt` is 250 evals for `pawnstar-v8.bin`, generated by the bullet `pawnstar_eval` example — regenerate it if you ship a new net); and (2) the **shipped int8 output layer** (`Network::Evaluate`) stays within 40 cp of that reference (measured max 26 cp — the SCReLU-into-8-bits requant). With no arguments it is a no-op.
...
- **Architecture**: a perspective net with bullet's `ChessBuckets` feature set — `768` inputs per (perspective, king bucket) → `1024` feature transformer (shared across both perspectives), SCReLU activation, `concat[stm | ntm] = 2048 → 1` output (centipawns, side-to-move relative). The shipped net uses a **single weight bank — no king buckets** (plain Chess768). Constants: `kHiddenSize=1024`, `kNumKingBuckets=1`, `kFeatureRows=768*1`, `kQA=255`, `kQB=64`, `kScale=400`.
...
- **King buckets**: the shipped net uses a single bank (`kKingBucketMap` all-zero → no king buckets). The mechanism generalises: with multiple banks each perspective would select one by *its own* king's square (in that perspective's orientation) via the 64-entry `kKingBucketMap`, kept byte-identical to the `ChessBuckets::new` array in `tools/bullet/pawnstar.rs`/`pawnstar_eval.rs`.
...
- **Accumulator** (`nnue::Accumulator`, two perspectives): maintained **incrementally** on `SearchState` across make/undo. `Network::Update`'s common case (no king-bucket crossing — always, for the single-bank net) finds the changed squares straight from the parent/child **colour bitboards** — a move touches only 2–4 squares, with captures/EP/castling/promotion all handled uniformly — and applies just those feature-column deltas to both perspectives (no per-piece-type scan). A king move that **crosses a bucket boundary** changes that whole perspective's bank, so `Update` rebuilds that one perspective from scratch (`RefreshSide`) and diffs the other (`DiffSide`) — at most one king moves per ply, so at most one side refreshes (with the shipped single-bank net, crossings never occur). `Network::Refresh` rebuilds both at the root (seeded with `feature_bias`). The colour-XOR delta is bit-identical to a per-piece diff (same columns; int16 accumulator add/sub is order-independent), and `test_nnue_incremental` checks the incremental accumulator stays bit-identical to a full refresh (random trees, including bucket crossings on multi-bank nets).
