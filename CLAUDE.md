...
**Quiescence search** — `SearchState::SearchQuiescent` ([search_state.cpp](src/search_state.cpp)) searches captures only (no transposition table — it does not probe or store). It applies SEE pruning: a capture with negative SEE that does not give check is skipped rather than searched.
