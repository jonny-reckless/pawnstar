...
> **Branch note (`kingbuckets-4`):** this branch is the king-buckets experiment — the engine and trainer
> use bullet's king-bucketed `ChessBuckets` feature set (4 buckets) instead of `Chess768`. The
> description below reflects this branch's code. **`main` still ships the Chess768 512-wide
> `nnue/pawnstar-v4.bin`**; a king-bucket net is not yet shipped (it can't load a Chess768 net, so the
> startup default falls back to the HCE here until a `kb` net is trained — see [nnue/README.md](nnue/README.md)).
> Keep this only if the SPRT vs v4 wins; otherwise the branch is abandoned like `bignet-1024` (1024-wide).
...
- **Weights** are loaded directly from the [bullet](https://github.com/jw1912/bullet) trainer's native quantised `.bin` (headerless, tightly packed little-endian `int16`: `feature_weights[768*4*512]` laid out bucket-major, `feature_bias[512]`, `output_weights[1024]`, scalar `output_bias`; bullet pads the trailing tensor to 64 bytes, which the loader tolerates). The forward pass and dequantisation mirror bullet's example exactly; correctness is verified by `test_nnue`. The loader size-gate only rejects nets that are *too small*, so a build of one architecture can silently misread a *larger* net — always build the engine/tests at the net's width before verifying.
- **Enabling**: NNUE is **on by default** — `main.cpp` loads `nnue/pawnstar-v4.bin` (cwd-relative, like the book) and enables NNUE at startup, falling back to the HCE if the net is absent or the wrong architecture. Override via env (`PAWNSTAR_EVALFILE=<path>` for a different net, `PAWNSTAR_NNUE=0` to disable) or UCI `setoption` (`EvalFile`, `UseNNUE`; UseNNUE is advertised default true). The network is a single `nnue::Network` owned by the `Game`, read-only after `Load` and shared by all Lazy SMP threads, which call only its `const` methods.
...
`main` ships one net ([nnue/README.md](nnue/README.md) §7): `nnue/pawnstar-v4.bin`, a **512-wide Chess768** net trained on public PlentyChess data that *beats* the HCE by a wide margin and is **on by default**. Lineage (all SPRT-measured): self-play labels lost to HCE (label quality was the lever); ~12× more data gave nothing (the 256 net was capacity-limited); doubling width 256→512 added +55 Elo fixed-depth / +71 time-control (**v4**); doubling again 512→1024 gave **~0 fixed-depth and −74 on the clock** (rejected — 60M saturates 512). The current experiment (this branch) adds **king buckets** to the 512 arch, pending SPRT vs v4. Incremental accumulator + AVX2 SIMD keep NNUE fast enough to win on the clock, not just at equal depth. Nets of different architecture/width are size-gated and not cross-loadable, so each shipped arch re-ships its net + a matching `test/nnue_reference.txt`.
