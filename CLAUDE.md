...
- **Loading**: NNUE is the only evaluator, so a net is **required** — `main.cpp` loads `nnue/pawnstar-v7.bin` (cwd-relative, like the book) at startup. If the file can't be loaded (absent, wrong cwd, wrong architecture) it **falls back to a copy of the net embedded in the binary** (`src/embedded_net.S`, `.incbin`'d at build time and linked into the engine only), so the engine always has a working evaluator; it exits only if **both** the file and the embedded fallback fail (a corrupt build). Override the path via env (`PAWNSTAR_EVALFILE=<path>`) or load a different net at runtime via UCI `setoption name EvalFile value <path>`. The embedded fallback is loaded via `Network::LoadFromMemory`, which shares the exact header detection/validation of `Network::Load`. There is no `UseNNUE` option (nothing to toggle to). The network is a single `nnue::Network` owned by the `Game`, read-only after load and shared by all Lazy SMP threads, which call only its `const` methods.
