...
**Search** — `game_t` ([include/game.h](include/game.h)) owns all shared search infrastructure: transposition table (64 MB, 256-stripe mutex), history heuristic table (`_Atomic uint32_t counts[MAX_PLY × 4096]`), opening book, chess clock, position stack, cancel flag (`atomic_bool is_cancel_pending`), and a `thread_pool_t` (NumCPU persistent threads). Search runs on a background `thrd_t` so UCI `stop` can interrupt it.

`search_state_t` ([include/search_state.h](include/search_state.h)) is per-thread search state: a copy-make `pos_stack` (seeded with the position at the fork point), a compact `hash_stack` of `{hash, half_move_clock}` entries for repetition detection (16 bytes × n vs 160 bytes × n for full positions), a `node_count`, and a pointer to the shared `game_t`. Allocated from a slab pool (`slice_allocator_t` in [include/slice_allocator.h](include/slice_allocator.h)) for O(1) alloc/free during parallel search.

The search stack ([src/search_root_node.c](src/search_root_node.c), [src/search_alphabeta.c](src/search_alphabeta.c), [src/search_quiescent.c](src/search_quiescent.c)) uses iterative deepening from depth 3, PVS (principal variation search), null-move pruning (R=4, guarded by: null-window, not a null-move position, not in check, not in endgame, PST score ≥ beta+100), late-move reduction (LMR: −1 ply from move index >3 when depth>2, null-window, non-capture, non-pawn; −2 from move index >6; −3 from move index >6 with zero history count), and check/promotion extensions. Move ordering: TT move first, then MVV-LVA combined with history heuristic.

**Young Brothers Wait (YBW)** — After searching 3 moves serially at a null-window node with depth ≥ 4 and available pool slots, remaining moves are dispatched to the thread pool. Each dispatch calls `thread_pool_try_acquire` (non-blocking) and stops if no slots are free. Each worker gets its own `search_state_t` forked from the parent and signals a shared `atomic_bool cutoff` on beta cutoff; the pool thread frees the `search_state_t` before posting its done semaphore. Only the main (serial) search dispatches workers — workers never sub-spawn.
