...
./build/test_bratko_kopec_nnue nnue/pawnstar-v6.bin   # 24 Bratko-Kopec positions searched with NNUE; reports moves solved
./build/test_nnue nnue/pawnstar-v6.bin test/nnue_reference.txt   # NNUE inference cross-check vs trainer
./build/test_nnue_incremental nnue/pawnstar-v6.bin              # incremental accumulator == full refresh
```

`make check` runs the NNUE tests against the shipped `nnue/pawnstar-v6.bin` (the Makefile passes the net via `$(wildcard …)`, so the checks degrade to a green no-op only if the net/reference are absent):
- `test_nnue` (`test/nnue_test.cpp`) takes `<net.bin> <reference.txt>` and checks the engine's NNUE eval against the trainer's reference evals (within ±2 cp; the checked-in `test/nnue_reference.txt` is 250 evals for `pawnstar-v6.bin`, generated by the bullet `pawnstar_eval` example — regenerate it if you ship a new net). With no arguments it is a no-op.
...
- `test_bratko_kopec_nnue` (`test/bratko_kopec_nnue.cpp`) takes `<net.bin> [depth]` and searches the 24 BK positions (single-threaded for determinism) with NNUE, reporting how many find a documented best move (currently 18/24 at depth 8). It gates only on search correctness (a legal move for every position), not the solve count, so a retrained net cannot break the build; no-op without a net.
...
- **Loading**: NNUE is the only evaluator, so a net is **required** — `main.cpp` loads `nnue/pawnstar-v6.bin` (cwd-relative, like the book) at startup and **exits with an error** if it cannot (absent or wrong architecture). Override the path via env (`PAWNSTAR_EVALFILE=<path>`) or load a different net at runtime via UCI `setoption name EvalFile value <path>`. There is no `UseNNUE` option (nothing to toggle to). The network is a single `nnue::Network` owned by the `Game`, read-only after `Load` and shared by all Lazy SMP threads, which call only its `const` methods.
