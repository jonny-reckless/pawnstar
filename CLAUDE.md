...
```bash
make check    # builds tests then runs all six suites; returns non-zero on any failure
```
...
Because NNUE scores differ from the handcrafted scores, the BK test is **handcrafted-eval only** — regenerate BK references with `UseNNUE` off and do not retune them for NNUE.
